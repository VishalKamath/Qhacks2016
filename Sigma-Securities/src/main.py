...
class Predictor(object):
    ...
    def perform_analysis(self, newsId, company_name, news_link):
        ...
        # Depending on Twitter API scraping, if positive sentiment detected by Alchemy API, proceed with further analysis.
    ...
    def perform_financial_analysis(self, newsId, company_name, sentiment, sentiment_score):
        ...
        # New thread will start so that after 5 minutes a stock price will be determined.
...
def checkIfValidTweet(tweetData):
        news_link = ''
        company_name = 'TSLA'
        #Data will scrape information from news articles embedded in tweets. This verifies that the article is important to our source. Searching for string AAPL is what's used now.
        
        # Validate tweet contains relevant information to us.
        ...
        # Link Parsing magic over here.
    
        # Verify that if a new tweet is being scanned with data we have already collected it is ignored. Preserve data integrity.
    
        if((news_link not in Predictor.links_repository) and news_link != '' and company_name != 'TSLA'):
...
logger.debug("Connected to Twitter API and listening..")

#test_tweet_text = "TSLA shares due for price rise, stock is highly undervalued with high potential. https://t.co/jjDDUc0BiN"  
